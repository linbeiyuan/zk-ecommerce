...
class YonghuUpdate(APIView):
    def post(self, request):
        ...
        jf=data['jf']
        id = data['id']
        Yonghu.objects.filter(id=id).update(
        id=id,
        yonghuming=yonghuming,
        mima=mima,
        shoujihao=shoujihao,
...
# 删除
class YonghuDelete(APIView):
    def post(self, request):
        data = request.data
        for id in data:
            Yonghu.objects.filter(id=int(id)).delete()
        return Response({'code': 0, 'msg': '删除成功'})

# 查询详情
class YonghuInfo(APIView):
    def get(self, request, id):
        id = id
        entity = Yonghu.objects.filter(id=int(id)).all()
        entityser = YonghuSer(entity, many=True)
        return Response({'code': 0, 'data': entityser.data[0]})

# 分页
class YonghuList(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量

        if page is None or limit is None:
            return YonghuListNotPage(request)
        else:
            return YonghuListPage(request)
...
def YonghuListPage(request):
    ...
    query_params = {
                        'id__icontains': request.query_params.get('id'),
                                ...
                                'dizhi__icontains': request.query_params.get('dizhi'),
    ...
    queryset = Yonghu.objects.filter(q_objects).all()
    count = Yonghu.objects.filter(q_objects).count()
    ...
    result = YonghuSer(items, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

def YonghuListNotPage(request):
    userid = request.query_params.get('userid')
    query_params = {
                        'id__icontains': request.query_params.get('id'),
                                'addtime__icontains': request.query_params.get('addtime'),
    ...
    queryset = Yonghu.objects.filter(q_objects).all()
    count = Yonghu.objects.filter(q_objects).count()
    result = YonghuSer(queryset, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

# 分页
class YonghuPage(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量
        sort = request.query_params.get('sort') # 排序
...
class LoginYonghu(APIView):
    def post(self, request):
        ...
        entity = Yonghu.objects.filter(yonghuming=username, mima=password).first()
        if entity:
...
# 用户获取session
class YonghuSession(APIView):
    def get(self, request):
        token = request.META.get('HTTP_TOKEN', 'No token provided')
        session = myjwt.jwt_decode(token)['data']['userid']
        users = Yonghu.objects.filter(id=session).all()
        user_ser = YonghuSer(users, many=True)
        user_ser.data[0]['role'] = 'yonghu'
