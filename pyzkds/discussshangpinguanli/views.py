...
# 保存
class DiscussshangpinguanliSave(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            reply = None
        Discussshangpinguanli.objects.create(
            refid=refid,
            userid=userid,
            nickname=nickname,
            content=content,
...
# 修改
class DiscussshangpinguanliUpdate(APIView):
    def post(self, request):
        ...
        reply=data['reply']
        id = data['id']
        Discussshangpinguanli.objects.filter(id=id).update(
        id=id,
        refid=refid,
        userid=userid,
        nickname=nickname,
...
# 删除
class DiscussshangpinguanliDelete(APIView):
    def post(self, request):
        data = request.data
        for id in data:
            Discussshangpinguanli.objects.filter(id=int(id)).delete()
        return Response({'code': 0, 'msg': '删除成功'})

# 查询详情
class DiscussshangpinguanliInfo(APIView):
    def get(self, request, id):
        id = id
        entity = Discussshangpinguanli.objects.filter(id=int(id)).all()
        entityser = DiscussshangpinguanliSer(entity, many=True)
        return Response({'code': 0, 'data': entityser.data[0]})

# 分页
class DiscussshangpinguanliList(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量

        if page is None or limit is None:
            return DiscussshangpinguanliListNotPage(request)
        else:
            return DiscussshangpinguanliListPage(request)
...
def DiscussshangpinguanliListPage(request):
    ...
    result = DiscussshangpinguanliSer(items, many=True)
...
def DiscussshangpinguanliListNotPage(request):
    ...
    queryset = Discussshangpinguanli.objects.filter(q_objects).all()
    count = Discussshangpinguanli.objects.filter(q_objects).count()
    result = DiscussshangpinguanliSer(queryset, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

# 分页
class DiscussshangpinguanliPage(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量
        sort = request.query_params.get('sort') # 排序
