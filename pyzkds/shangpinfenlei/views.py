...
# 保存
class ShangpinfenleiSave(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            fenleimingcheng = None
        try:
            fenleitupian=data['fenleitupian']
            if fenleitupian == '':
                fenleitupian = None
        except KeyError:
            fenleitupian = None
            ...
            fenleitupian = None
        Shangpinfenlei.objects.create(
            fenleimingcheng=fenleimingcheng,
            fenleitupian=fenleitupian,
        )
        return Response({"code": 0, 'msg': '添加成功'})

# 修改
class ShangpinfenleiUpdate(APIView):
    def post(self, request):
        ...
        fenleitupian=data['fenleitupian']
        id = data['id']
        Shangpinfenlei.objects.filter(id=id).update(
        id=id,
        fenleimingcheng=fenleimingcheng,
        fenleitupian=fenleitupian,
        )
...
# 删除
class ShangpinfenleiDelete(APIView):
    def post(self, request):
        data = request.data
        for id in data:
            Shangpinfenlei.objects.filter(id=int(id)).delete()
        return Response({'code': 0, 'msg': '删除成功'})

# 查询详情
class ShangpinfenleiInfo(APIView):
    def get(self, request, id):
        id = id
        entity = Shangpinfenlei.objects.filter(id=int(id)).all()
        entityser = ShangpinfenleiSer(entity, many=True)
        return Response({'code': 0, 'data': entityser.data[0]})

# 分页
class ShangpinfenleiList(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量

        if page is None or limit is None:
            return ShangpinfenleiListNotPage(request)
        else:
            return ShangpinfenleiListPage(request)
...
def ShangpinfenleiListPage(request):
    ...
    query_params = {
                        'id__icontains': request.query_params.get('id'),
                                ...
                                'fenleitupian__icontains': request.query_params.get('fenleitupian'),
                    }
    q_objects = Q()
    ...
    queryset = Shangpinfenlei.objects.filter(q_objects).all()
    count = Shangpinfenlei.objects.filter(q_objects).count()
    ...
    result = ShangpinfenleiSer(items, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

def ShangpinfenleiListNotPage(request):
    userid = request.query_params.get('userid')
    query_params = {
                        'id__icontains': request.query_params.get('id'),
                                'addtime__icontains': request.query_params.get('addtime'),
    ...
    queryset = Shangpinfenlei.objects.filter(q_objects).all()
    count = Shangpinfenlei.objects.filter(q_objects).count()
    result = ShangpinfenleiSer(queryset, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

# 分页
class ShangpinfenleiPage(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量
        sort = request.query_params.get('sort') # 排序
