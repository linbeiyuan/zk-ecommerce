...
# 保存
class JifenshangdianSave(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            shangpintupian = None
        try:
            shangpinjianjie=data['shangpinjianjie']
            if shangpinjianjie == '':
                shangpinjianjie = None
        except KeyError:
            shangpinjianjie = None
        except ValueError:
            shangpinjianjie = None
...
# 修改
class JifenshangdianUpdate(APIView):
    def post(self, request):
        ...
        jifen=data['jifen']
        id = data['id']
        Jifenshangdian.objects.filter(id=id).update(
        id=id,
        shangpinmingcheng=shangpinmingcheng,
        shangpintupian=shangpintupian,
        shangpinjianjie=shangpinjianjie,
...
# 删除
class JifenshangdianDelete(APIView):
    def post(self, request):
        data = request.data
        for id in data:
            Jifenshangdian.objects.filter(id=int(id)).delete()
        return Response({'code': 0, 'msg': '删除成功'})

# 查询详情
class JifenshangdianInfo(APIView):
    def get(self, request, id):
        id = id
        entity = Jifenshangdian.objects.filter(id=int(id)).all()
        entityser = JifenshangdianSer(entity, many=True)
        return Response({'code': 0, 'data': entityser.data[0]})

# 分页
class JifenshangdianList(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量

        if page is None or limit is None:
            return JifenshangdianListNotPage(request)
        else:
            return JifenshangdianListPage(request)
...
def JifenshangdianListPage(request):
    ...
    query_params = {
                        'id__icontains': request.query_params.get('id'),
                                ...
                                'shangpinjianjie__icontains': request.query_params.get('shangpinjianjie'),
                                'shangpinxiangqing__icontains': request.query_params.get('shangpinxiangqing'),
                                'jifen__icontains': request.query_params.get('jifen'),
                    }
    q_objects = Q()
    ...
    queryset = Jifenshangdian.objects.filter(q_objects).all()
    count = Jifenshangdian.objects.filter(q_objects).count()
    ...
    result = JifenshangdianSer(items, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

def JifenshangdianListNotPage(request):
    userid = request.query_params.get('userid')
    query_params = {
                        'id__icontains': request.query_params.get('id'),
                                'addtime__icontains': request.query_params.get('addtime'),
    ...
    queryset = Jifenshangdian.objects.filter(q_objects).all()
    count = Jifenshangdian.objects.filter(q_objects).count()
    result = JifenshangdianSer(queryset, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

# 分页
class JifenshangdianPage(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量
        sort = request.query_params.get('sort') # 排序
