<script setup lang="ts">
    ...
    function open(title:string,id:any,type:string,formData?:any)
    {
        ...
        if (state.type == 'cross') {
            ...
            for (var o in obj) {
                                                                                                                                    if (o == 'yonghuming') {
                            state.formData.yonghuming = obj[o];
                            state.roisfalg.yonghuming = true;
                            continue;
                        }
                                                                                if (o == 'mima') {
                            ...
                            state.formData.shangjiamingcheng = obj[o];
                            state.roisfalg.shangjiamingcheng = true;
                            continue;
                        }
                                                                                if (o == 'money') {
                            state.formData.money = obj[o];
                            state.roisfalg.money = true;
                            continue;
                        }
                                                                                if (o == 'jf') {
                            state.formData.jf = obj[o];
                            state.roisfalg.jf = true;
                            continue;
                        }
                                                }
    ...
    function submitForm(){
        formRef.value?.validate((valid:boolean)=>{
            ...
            state.loadding=true;
            request({
                url:`shangjia/${!state.formData.id ? "save" : "update"}`,
                method:'post',
                data:state.formData
            }).then(({data})=>{
    ...
    <el-dialog :title="`${title}`" v-model="visible"
        ...
        <div v-loading="loadding">
            ...
            >
                        ...
                        <el-form-item label="用户名" prop="yonghuming" :rules="{required: true, message: '用户名为必填项！', trigger: 'blur'}">
                            <el-input v-model="formData.yonghuming" maxlength="30" clearable show-word-limit placeholder="请输入用户名" :readonly="roisfalg.yonghuming"/>
                        </el-form-item>
                        ...
                        <el-form-item label="商家名称" prop="shangjiamingcheng" :rules="{required: true, message: '商家名称为必填项！', trigger: 'blur'}">
                            <el-input v-model="formData.shangjiamingcheng" maxlength="30" clearable show-word-limit placeholder="请输入商家名称" :readonly="roisfalg.shangjiamingcheng"/>
                        </el-form-item>
