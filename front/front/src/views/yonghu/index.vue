    ...
    function getDataList(){
      ...
      const params={
        ...
        limit: state.page.size,
                                                    ...
                                                    if (state.searchForm.yonghuming != '' && state.searchForm.yonghuming != undefined) {
            params['yonghuming'] = '%' + state.searchForm.yonghuming + '%'
          }
...
<template>
  <div class="list-page">
    <div class="list-container">
      <!-- 页面标题 -->
      <h2 class="list-page-title">用户</h2>
      ...
      <div class="list-search">
        ...
        <div class="list-search-input">
          ...
          <input type="text" placeholder="用户名" v-model="searchForm.yonghuming">
        </div>
      ...
      <div class="list-grid">
        <router-link
          ...
          :key="index"
          :to="`/yonghudetail/${item.id}`"
          class="list-card"
        >
          <div class="list-card-info">
            <h3 class="list-card-title">{{ item.yonghuming }}</h3>
          </div>
          <div class="list-card-info">
            <h3 class="list-card-title">{{ item.mima }}</h3>
          </div>
          <div class="list-card-info">
            <h3 class="list-card-title">{{ item.shoujihao }}</h3>
          </div>
          <div class="list-card-info">
            <h3 class="list-card-title">{{ item.dizhi }}</h3>
          </div>
        </router-link>
      </div>
