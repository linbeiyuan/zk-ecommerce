<script setup lang="ts">
  ...
  const AddEdit=defineAsyncComponent(()=>import('@/views/shangpinguanli/add-edit.vue'));
      const CrkEdit=defineAsyncComponent(()=>import('@/views/crk/add-edit.vue'));
  ...
  function getDateList(){
    ...
    if (users && users.role === 'shangjia') {
      params['userid'] = users.id
                                                            ...
                                                            if (state.query.shangpinmingcheng != '' && state.query.shangpinmingcheng != undefined) {
              params['shangpinmingcheng'] = '%' + state.query.shangpinmingcheng + '%'
            }
                                                                                                      if (state.query.dianpumingcheng != '' && state.query.dianpumingcheng != undefined) {
              params['dianpumingcheng'] = '%' + state.query.dianpumingcheng + '%'
            }
                                                                                                                                                    request({
      url:'shangpinguanli/page',
      method:'get',
      params
    }).then(({data})=>{
...
const multipleSelection = ref<any[]>([]);
  ...
  const crkRef=ref();
  const crk=(row:any,tablename:string)=>{
    crkRef.value.open('出入库','add',tablename,row);
  }
</script>
<template>
        ...
        <div class="layout-padding">
            <el-form inline :model="query" label-suffix=":">
                        ...
                        <el-form-item label="商品名称"  prop="shangpinmingcheng">
                            <el-input v-model="query.shangpinmingcheng" maxlength="100" placeholder="请输入商品名称" clearable style="width: 200px;"/>
                        </el-form-item>
                  ...
                  <div class="button-container">
                    ...
                    <el-button  class="button button-6" v-if="isAuth('shangpinguanli','删除')" icon="ele-Delete" @click="listdel()" :disabled="isdel" type="danger">多选删除</el-button>
                    <el-button class="button button-8" v-if="isAuth('shangpinguanli','新增')" icon="ele-Plus" type="success" @click="add()">新增</el-button>
                                                                                                          </div>
            ...
            color: '#ffffff'
                ...
                <el-table-column type="selection" width="55" />
                                                                                                                                                                                                ...
                                                                                                                                                                                                <el-table-column header-align="center"  align="center" prop="shangpinmingcheng" label="商品名称">
                            ...
                            <el-table-column header-align="center"  align="center" prop="shangpintupian" label="商品图片">

                                <template #default="{row}">
                                    <img v-if="row.shangpintupian" :src="row.shangpintupian" width="100" height="100">
                                    <div v-else>无图片</div>
                                </template>
                                    ...
                                    <template #default="{row}">
                                        {{row.price}}
                                                                                                                                                                                                ...
                                                                                                                                                                                                <el-table-column header-align="center"  align="center" prop="onelimittimes" label="单限">
                                ...
                                <el-table-column  v-if="isAuth('shangpinguanli','审核')" header-align="center"  align="center" prop="sfsh" label="审核">
                    ...
                    <template #default="{row}">
                                                  <el-button class="button button-8" v-if="isAuth('shangpinguanli','出入库')" icon="ele-Edit" @click.stop="crk(row,'shangpinguanli')" type="warning">出入库</el-button>
                                                                        <el-button class="button button-8" v-if="isAuth('shangpinguanli','修改')" icon="ele-Edit" @click.stop="handleEdit(row)" type="warning">修改</el-button>
                        <el-popconfirm  v-if="isAuth('shangpinguanli','删除')"  width="auto" @confirm="handleDelete(row)"  :title="`确定永久删除吗？`">
                            <template #reference>
          ...
          <CrkEdit ref="crkRef" @refresh="getDateList"></CrkEdit>
          </div>
    </el-card>
