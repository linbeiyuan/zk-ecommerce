<script setup lang="ts">
  ...
  const AddEdit=defineAsyncComponent(()=>import('@/views/discussshangpinguanli/add-edit.vue'));
  ...
  function getDateList(){
    ...
    if (users && users.role === 'shangjia') {
      params['shangjia_userid'] = users.id
                                                                                                                          ...
                                                                                                                          request({
      url:'discussshangpinguanli/page',
      method:'get',
      params
    }).then(({data})=>{
  ...
  function handleDelete(row:any){
    ...
    request({
      url:`discussshangpinguanli/delete`,
      method:'post',
      data:[row.id]
    }).then(({data})=>{
        ...
        <div class="layout-padding">
            <el-form inline :model="query" label-suffix=":">
                  ...
                  <div class="button-container">
                    ...
                    <el-button  class="button button-6" v-if="isAuth('discussshangpinguanli','删除')" icon="ele-Delete" @click="listdel()" :disabled="isdel" type="danger">多选删除</el-button>
                    <el-button class="button button-8" v-if="isAuth('discussshangpinguanli','新增')" icon="ele-Plus" type="success" @click="add()">新增</el-button>
                                                                                                          </div>
            ...
            color: '#ffffff'
                ...
                <el-table-column type="selection" width="55" />
                    ...
                    <template #default="{row}">
                                                                        <el-button class="button button-8" v-if="isAuth('discussshangpinguanli','修改')" icon="ele-Edit" @click.stop="handleEdit(row)" type="warning">修改</el-button>
                        <el-popconfirm  v-if="isAuth('discussshangpinguanli','删除')"  width="auto" @confirm="handleDelete(row)"  :title="`确定永久删除吗？`">
                            <template #reference>
